...
The current repository implements and qualifies the pre-release single-node and clustered profiles: Deployment/API-key and credential custody, a same-origin terminal-first Workspaces/Hosts/Credentials/Audit/Deployment shell with bounded page-memory workspace tabs, complete Machine/Relay lifecycle control, active-Machine credential rebind for future SSH children, Relay enrollment/tunnel/actual ownership, read-only and writable SSH/tmux projection, automatic visible-writer viewport resize, owner-local Browser writer coordination across tabs, symmetric clustered remote-owner/internal-WSS routing, safe audit/metrics, repeatable recovery evidence, the documented Linux/tmux/login-shell matrix, and the production Server image. Version `0.0.1` was the initial tag-driven, CI-published evaluation release. Version `0.0.2` added independently buildable Server and Relay source packages on crates.io. Version `0.0.3` added the qualified terminal-first Web shell, bounded page-memory workspaces, and automatic visible-writer resize. Version `0.0.4` added first-enrollment-confirmed SSH host-key pinning plus qualified responsive Web layouts from wide desktop through small mobile viewports. Version `0.0.5` is the current evaluation release and adds bounded same-origin API-key persistence with fresh load-time validation and explicit failure cleanup; the Server crate does not replace the complete image/archive because it does not embed the Web build. No production-supported version has been released; do not extend the qualification claim beyond the documented profiles.

## Deployment access

- One explicit `OWLMUX_API_KEY` with prefix `owlmux_sk_v1_` and the canonical unpadded base64url encoding of exactly 32 operator-generated random bytes grants complete access to every Web/API resource and attachment in the deployment. Deployment is the sole human/API trust boundary; do not subdivide it into identities, delegated grants, per-resource authorization, alternate login methods, or Server-issued Browser sessions.
- Browser accepts only the exact canonical API-key shape, removes malformed saved values without transport, and freshly verifies a valid candidate under a bounded deadline. After success it attempts to store the raw key under the one fixed versioned `localStorage` entry for the Deployment origin, keeps the active client copy only in page memory, sends Bearer on every protected HTTP request, and uses it for attachment first-frame authentication. Explicit logout, HTTP 401, or attachment authentication failure MUST clear every page-memory copy and attempt saved-key removal; a storage write/removal failure MUST be visibly reported and removal failure MUST instruct the operator to clear origin site data. Network/unavailability/validation-timeout failure MUST retain any saved candidate for retry. It MUST NOT persist the key in cookies, `sessionStorage`, IndexedDB, Cache Storage, service workers, URLs, logs, analytics, or any other serialized state. Reload, page close, or navigation away cancels pending verification and clears workspace/page state but retains a valid saved key; late verification completion has no effect.
...
- API-key rotation replaces the sole Deployment value through a controlled all-node drain/stop, waits for prior leases to expire, increments the configuration epoch/proof, and restarts coherent nodes. The old key, nodes, and connections immediately cease to work; fresh HTTP/WebSocket authentication clears invalid page authority and triggers saved-key removal with visible cleanup-failure handling. Ordinary unchanged-key node/page restart may restore the same-origin saved key and revalidate it. Rotation has no grace key, online mutation, per-node transition, or Server-side session state.
...
- Non-recoverable enrollment values are stored as digests, not encrypted. Browser may persist only the raw Deployment API key in its one fixed same-origin `localStorage` entry; no Server session, workspace, terminal, owner, or operation state is persisted with it.
...
- Treat OpenSSH as the initial constrained SSH client. Use a dedicated Server-owned config, exclusive explicit `known_hosts` inputs, `IdentitiesOnly`, and no ambient agent for target authentication. For only a Machine's first enrollment, run a credential-free Ed25519-only `accept-new` preflight against an isolated empty `known_hosts`, require Relay to canonicalize the key and locally recompute the advertised fingerprint, present that derived fingerprint for exact interactive `yes` without surrounding whitespace or exact expected-fingerprint automation, then run strict proof on a separate stream. First activation is the sole absent-to-confirmed host-key write; every later path uses `StrictHostKeyChecking=yes` against the immutable pin. Browser input never chooses SSH options, destinations, identities, remote commands, or tmux syntax. Enrollment verification uses one fixed no-tmux `VerifySshAccess` that emits one constant bounded marker and exits zero; probe/create/attach use their own closed typed entry operations. All share one reviewed shell-literal renderer across a qualified login-shell matrix, never a generic command or target wrapper.
- Treat tmux 3.2a as the minimum target compatibility baseline, not as a promise that every newer version works. Before a writable workspace, run bounded capability probes and reject a small release-maintained known-bad denylist. Use focused real-target compatibility evidence across the minimum, maintained distribution packages, one current release, and qualified login shells rather than running the complete product E2E for every profile or maintaining a runtime package allowlist or Cartesian target manifest. Detect and explain missing or incompatible target tmux, but never invoke a package manager or install, upgrade, downgrade, patch, configure, or repair it.
...
- Relay enrollment uses a token-only bounded first frame. Successful bounded digest resolution atomically consumes the token and creates one deadline-bounded durable `Verifying` attempt before setup. Relay persists its candidate ID/key before enrollment and the returned Deployment/Machine IDs before setup. Setup contains no host-key candidate. Fresh Relay challenge, optional first-use host discovery/confirmation, and strict verified proof stay only in bounded memory on that same live accepting connection; there is no persisted coordinator/challenge/proof or resume on another connection. A first enrollment permits exactly two ordered provisional streams, discovery then strict proof; an already pinned Machine permits one strict proof stream. Final activation locks `DEPLOYMENT` then the attempt/Machine/credential/executing node and, under partial unique constraints, rechecks post-lock PostgreSQL time, exact Serving lease/config/build/protocol, and that no other active binding uses the Relay ID/key before atomically pinning the first confirmed host key when absent and inserting the binding. Raw token material never crosses a Server hop. Failure returns to tokenless `Pending` without changing host trust and requires explicit issuance.
...
- CI is the validation authority. The operator creates an immutable version tag only after the exact `main` commit completes CI successfully; release workflows trust that tag, do not query the Actions API, and must not duplicate lint or tests.
