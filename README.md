...
Then open `http://127.0.0.1:8080`, enter the disposable API key from `dev/server.env`, select **Open OwlMux**, and begin at Workspaces. Use Hosts to add or manage a fixed target scope and Credentials to copy its selected public key. A target administrator must install that generated public key before Relay enrollment can prove SSH access.
...
Run the isolated real PostgreSQL/Relay/OpenSSH/tmux acceptance path with versioned Node attachment-WebSocket clients and headless Chromium. It covers enrollment recovery, credential locking and active rebind, safe lifecycle/detail presentation, audit/metrics, the Workspaces/Hosts/Credentials journeys, React rename and safe Host rendering, page-memory search/tabs, multi-pane projection and xterm rendering under the product CSP, continuous snapshot/live cutover, binary live output, concurrent writer claim, same-page and mobile takeover, explicit session creation, literal input, observer geometry isolation, target-observed writer/observer client flags, stale-writer rejection, automatic target-authoritative viewport resize, projection refresh, route replacement, active re-enrollment, zero-session recovery, hard fencing, reload key/tab clearing, and target tmux survival. The matrix command repeats the complete path with Ubuntu 22.04 tmux 3.2a, Debian 12 tmux 3.3a under `dash`, Debian 13 tmux 3.5a, and checksum-pinned current upstream tmux 3.7b:
