...
Target tmux and sshd are authoritative. A compromised pinned target can emit malicious terminal data or control its shell. Host verification prevents silent routing to a different key but does not make the accepted host trustworthy.

Machine creation accepts no SSH host key. During only the first Relay enrollment, Server uses constrained OpenSSH with an isolated empty `known_hosts`, `StrictHostKeyChecking=accept-new`, no user-authentication credential, a fixed alias, and an Ed25519-only policy. Relay canonicalizes the discovered Ed25519 key, locally recomputes and displays its SHA-256 fingerprint, rejects any mismatch with the advertised fingerprint, and requires exact interactive `yes` with no surrounding whitespace or an exact locally checked expected fingerprint in non-interactive automation. A separate `StrictHostKeyChecking=yes` child proves the selected account and credential before one activation transaction pins the key. Any failure leaves the key absent. Re-enrollment and every attachment skip discovery and fail closed against that immutable pin; changing the target host key requires a new Machine.
