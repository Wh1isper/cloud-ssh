...
The Browser has no Server-issued session: after successful verification it attempts to save the Deployment API key in one fixed versioned same-origin `localStorage` entry, restores and revalidates a valid candidate under a bounded deadline on later loads, sends Bearer on each protected HTTP request, and authenticates each attachment WebSocket in its first bounded frame. At most 16 workspace tabs remain only in page memory, and internal SPA navigation preserves them. Reload, page close, or navigation away cancels pending verification and clears every tab but retains a valid saved key for fresh validation; logout or authentication failure always clears page authority, attempts saved-key removal, and requires re-entry. Storage failure is visibly reported, including a manual site-data cleanup warning when removal fails. An unchanged-key node or Browser restart can revalidate the saved candidate; coordinated key replacement makes the old candidate fail and trigger removal. None of these events affects target tmux.
