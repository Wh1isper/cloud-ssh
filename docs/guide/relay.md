...
The connection accepts exactly one setup frame covering the candidate Relay ID/public key, fixed endpoint, account, and exact protocol version. It contains no host key or host-key candidates. Server returns the selected SSH credential's public metadata separately, and the target administrator confirms in Relay CLI that this credential key is installed.

For a Machine that has never activated, Server then opens provisional stream 1 and runs system OpenSSH without a user-authentication credential. An empty isolated `known_hosts`, `StrictHostKeyChecking=accept-new`, fixed host alias, and Ed25519-only policy capture exactly the key presented by loopback sshd. Relay independently canonicalizes the Ed25519 key, recomputes its SHA-256 fingerprint, rejects any mismatch with Server's advertised value, and displays the locally derived value with the familiar SSH authenticity prompt. It accepts only exact interactive `yes` with no surrounding whitespace. Non-interactive first enrollment must supply `--expected-host-key-sha256 SHA256:...` and proceeds only on an exact match against that locally derived value; there is no unconditional acceptance flag. Relay echoes the exact accepted key, Server closes the stream, and no durable pin exists yet.

Server next opens provisional stream 2 and runs the closed fixed no-tmux `VerifySshAccess` operation with `StrictHostKeyChecking=yes`, the confirmed host key, and selected Machine credential. Only its exact constant marker followed by clean zero exit proves SSH host/account/key acceptance. For re-enrollment, Server skips discovery and confirmation, opens one strict proof stream against the existing durable pin, and fails closed on any mismatch.

Final activation locks `DEPLOYMENT` first, rechecks exact configuration/build/protocol, then locks the exact attempt, Machine/credential, and executing Server-incarnation row. Under that Deployment lock plus database partial unique constraints, post-lock PostgreSQL current time must show an unexpired attempt, no other active binding for the Relay ID/public key, and an exact Serving, lease-valid, config/build/protocol-current node. On first activation the transaction performs the only allowed absent-to-confirmed host-key write and creates the active Relay binding together; every later activation requires exact equality with the immutable pin. A query delayed across attempt/node lease expiry, drain, fence, replacement, active-identity conflict, or configuration change cannot activate durable trust. The accepting node then claims itself as current Machine owner before ordinary logical streams open. If activation or owner reporting is ambiguous, Relay closes and reconnects with the IDs/key it persisted before setup; it never replays the enrollment token, setup, or activation blindly.

Known setup/preflight/confirmation/proof/connection failure or attempt expiry returns the Machine to tokenless `Pending` through protected recovery without pinning or changing its host key; a new token must be issued explicitly. An expired crash residue may be recovered safely without resurrecting the old token. A Relay ID or Ed25519 public key may appear in at most one active Machine binding; OwlMux does not retain a permanent registry of invalidated Relay identities.

Active re-enrollment first fences the current owner, invalidates the old Relay binding, increments the route revision, and returns the same pinned-host/account/socket Machine to tokenless `Pending`; issuing a new one-use token is a following explicit action. It never rediscovers or accepts a changed target identity in place.
...
One route-scoped owner-local pointer identifies the current Browser writer, including across multiple same-page workspace tabs for one Machine. The product UI calls a free claim **Take control**, an occupied claim **Take over**, and the holder **You have control**. Claim/takeover, session creation, writer resize, observed window/pane selection, active-pane literal input, and projection hydration serialize through the same bounded dispatch barrier. Every control client atomically attaches read-only with `ignore-size`; a takeover makes the old client `ignore-size` before toggling it read-only, then uses tmux's dedicated read-only toggle to promote the claimant, removes `ignore-size`, resizes, and freshly hydrates it. There is no rows/columns form: only the current visible ready writer measures its viewport and xterm cell size and emits bounded, debounced, deduplicated resize intent. Observer and hidden tabs do not affect target geometry. Notifications observed while the barrier is busy become one coalesced dirty work item whose dispatch wait keeps its queue position while the workspace still handles demotion and close; they are never dropped or mixed across projection generations. Target mutation outcomes are exact success, known failure, or conservative ambiguous. OwlMux never retries or compensates an ambiguous input or mutation.
