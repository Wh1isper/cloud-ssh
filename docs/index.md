...
features:
  ...
  - title: One Deployment API key
    details: One Browser/API key grants complete access within one independent self-hosted trust domain; Browser attempts same-origin-only persistence after verification, revalidates it on load, and reports any save or cleanup failure instead of hiding it.
