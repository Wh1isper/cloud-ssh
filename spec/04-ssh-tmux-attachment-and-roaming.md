...
When no writer exists, the owner serializes concurrent claims and accepts the first. A later observer may explicitly select `Take over`; the same owner-local ordered dispatch path atomically replaces the pointer before accepting any later write. Every write is received on its authenticated attachment connection and is rejected immediately unless that exact connection is still the pointer, its workspace is current, and the node/owner/Machine fences remain valid. A separate writer token is neither sent nor stored.
...
- Each qualified login shell passes the closed remote-entry render/escape fixtures, and representative Relay-backed Browser E2E covers the complete journey without constructing or claiming a Cartesian target matrix.
