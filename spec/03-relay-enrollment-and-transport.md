...
stateDiagram-v2
    ...
    Verifying --> Active: Relay, host, account, attempt, and proof commit
...
A Machine is attachable only in `Active`. Advisory Relay reachability does not change durable lifecycle. Re-enrollment is explicit. From `Active`, the valid owner first closes its dispatch barrier; one durable transaction then invalidates the old Relay binding, increments `route_revision`, clears the exact owner, moves the Machine to tokenless `Pending`, and audits before a new token is explicitly issued. From `Disabled`, re-enrollment similarly retains the fixed Machine scope and enters tokenless `Pending`. Host/account/socket or host-key changes are never accepted merely because a reconnect or new setup presents different material.
...
1. create the Machine with fixed target scope;
...
Machine creation does not generate another private key. Ed25519 credential generation and encryption are owned by [06](06-storage-consistency-and-private-key-encryption.md#32-ssh-credentials). Public key and SHA-256 fingerprint may accompany the one-use result and remain readable; private material and encrypted envelopes never cross this boundary.
...
3. The same live enrollment connection remains on that accepting ingress process; there is no persisted coordinator, node selection, internal forwarding, or resumable attempt connection. The acceptance response supplies the immutable Deployment and Machine IDs known from the consumed token. Relay already generated and persisted a fresh candidate Relay ID plus Ed25519 key and MUST durably persist those returned IDs before continuing. Ingress then accepts exactly one bounded `enroll.setup` frame containing that candidate Relay ID/public key, fixed loopback endpoint, observed account, bounded host-key candidates, and the exact initial Relay protocol version. The token is never repeated. Before setup, proof, or database dispatch, ingress reads `CLOCK_BOOTTIME` and checks its local hard deadline. It validates setup against that exact attempt, creates a fresh in-memory proof challenge, and continues confirmation and independent SSH proof locally. Persistence failure closes the attempt before proof; it cannot activate.
4. A known setup/proof failure, deadline, connection loss, node drain/fence, or process loss leaves the Machine non-active and returns the durable expired/failed attempt to tokenless `Pending`; the API-key holder explicitly issues a new token before retry. No failure activates a route or transfers/resumes the attempt.
5. Final activation is one transaction that first locks `DEPLOYMENT` and rechecks exact current config/build/protocol, then locks the exact `Verifying` attempt, Machine/credential, and executing ingress's exact `SERVER_NODE` row. Under that still-held Deployment lock and the database partial unique constraints, it uses a fresh PostgreSQL `clock_timestamp()` sample, never transaction-start `now()` or pre-wait statement time, to require the attempt deadline and executing incarnation lease still valid, that incarnation still `Serving` with exact current config/build/protocol, the Machine/credential/route context still current, and that no other active binding uses the candidate Relay ID or public key before creating the active Relay binding and audit. Connection-local proof success is a required application precondition and is never represented as a database row. The proof challenge stays in this connection's bounded memory; it is never durable or resumable. A request delayed across lease expiry, drain, fence, incarnation replacement, or config transition cannot activate durable trust. The same ingress then claims itself as actual Machine owner under a fresh connection epoch before reporting `tunnel.active`. If activation commits but owner claim/reporting is lost, Relay reports an unknown/closed outcome and reconnects with the Deployment/Machine/Relay IDs and private key it durably stored before setup; no activation or tunnel bytes are replayed.
...
sequenceDiagram
    ...
    Relay->>SSHD: Observe local account and host-key candidates
    ...
    Relay->>Ingress: enroll.setup(Relay ID/key, endpoint, account, host candidates)
    Ingress->>Ingress: Validate setup and create in-memory proof challenge
    Ingress-->>Relay: Selected SSH public key and confirmation data
    Relay-->>User: Show account, Ed25519 public key, and fingerprint
    User->>SSHD: Install exact public key through external operations
    User->>Relay: Confirm readiness
    Relay-->>Ingress: Human readiness confirmation only
    Ingress->>Relay: Open one independent proof stream
    Relay->>SSHD: Connect fixed loopback endpoint
    Ingress->>SSHD: Run fixed VerifySshAccess for exact host/account/key
    SSHD-->>Ingress: Bound host/account proof
    Ingress->>DB: Lock attempt/Machine/credential/own node; DB-time revalidate and activate binding
...
Human confirmation permits proof to begin; it is not evidence that the key works. Only the exact constant marker followed by clean zero exit from the closed, fixed, no-tmux `VerifySshAccess` operation over end-to-end SSH proves acceptance by the selected account at the verified host. Failed or ambiguous proof leaves the Machine non-active and creates no OwlMux-owned target mutation to compensate.
...
machine.id == expected_host_identity.machine_id
...
| SSH host mismatch                                                         | Fail closed. Re-enrollment may replace Relay identity only and must prove the same expected host/account/socket; if target host identity changed, create a new Machine              |
...
- First successful token verification atomically consumes it and creates one deadline-bounded `Verifying` attempt before setup; Relay persists the returned Deployment/Machine IDs with its candidate identity before setup, and setup, proof, activation, and initial owner claim stay on the same live enrollment connection and accepting incarnation without a durable coordinator record. Known failure or expired crash residue returns to `Pending` without token resurrection; retry requires explicit issuance.
- Final activation atomically binds one Machine, Relay ID/key unused by another active binding, SSH host, account, endpoint, and selected Deployment credential only if the transaction locks and revalidates the executing ingress's PostgreSQL-time Serving lease/config plus exact `VerifySshAccess` success; delayed stale/fenced SQL or an active identity conflict cannot activate. That exact ingress then claims a monotonic Machine connection epoch before ordinary streams open, and a lost response is recoverable by authenticated reconnect without enrollment replay.
