...
The selection is valid only with the constrained boundary in [04](04-ssh-tmux-attachment-and-roaming.md): dedicated Server configuration, explicit isolated `known_hosts` inputs and a fixed host-key alias, first-enrollment-only `accept-new` discovery without user authentication, the exact Machine-selected Deployment credential for subsequent strict proof and attachment, `IdentitiesOnly`, no ambient agent for target authentication, disabled forwarding/prompts/PTY, cleaned environment, no local shell, bounded diagnostics, and local-only teardown. Relay streams reach OpenSSH through a bounded Server-owned fixed bridge rather than ambient SSH configuration or a browser-selected `ProxyCommand`.
...
Focused real-target checks keep compatibility evidence across tmux 3.2a, selected maintained distribution packages, one current upstream release, qualified login shells, control transcripts, backpressure, remote-entry escaping, and target-process survival. CI runs one representative single-node path instead of repeating the complete product E2E for every profile. This is evidence for the supported behavior, not a Cartesian product or a runtime allowlist. A newly observed incompatibility becomes a focused denylist entry with a regression fixture.
...
Protected HTTP uses the Deployment API key as Bearer on every request at whichever node ingress accepts it. Browser attempts to store a successfully verified candidate only in one fixed versioned same-origin `localStorage` entry, restores and revalidates a valid candidate under a bounded deadline on later loads, keeps the active-client copy in current page memory, and visibly reports storage failure. Because native WebSocket cannot set arbitrary Authorization, the attachment WebSocket uses one size-bounded `auth.api_key` first frame under a short deadline and allocates no Machine lookup, owner resolution, internal owner-WSS, or Attachment resources before success. URL/query/cookie/subprotocol transport and every other Browser persistence path are forbidden.
