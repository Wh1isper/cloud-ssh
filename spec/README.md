...
| Browser rendering, same-origin saved API key, page-memory workspaces, local preferences      | Browser profile and current page                                                                                                         | Saved raw key remains full Server-verified authority; other Browser state is never durable target truth or Server authority       |
...
09. **Bounded Browser key persistence.** Protected HTTP carries Bearer every request; Browser WebSocket authenticates only with one bounded first frame; one successfully verified raw key may persist only in the fixed versioned same-origin `localStorage` entry, while malformed values are rejected and storage failures are visibly reported. No URL/cookie/subprotocol or alternate storage credential transport exists.
